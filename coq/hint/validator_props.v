...
Lemma eq_check_lsv_prop
  ...
  (Hwfm: wf_sb_mi gmax alpha mem1 mem2)
  (Hgl: globals_equivalent alpha gmax (Globals cfg1) (Globals cfg2))
  (H1: values2GenericValueExt (CurTargetData cfg1) v1 olc1 lc1 (Globals cfg1) = ret gvs1)
  (H2: values2GenericValueExt (CurTargetData cfg2) v2 olc2 lc2 (Globals cfg2) = ret gvs2),
...
Proof.
  ...
  - infrule_tac.
    ...
    remember (values2GenericValueExt (CurTargetData cfg1) v1 olc1 lc1 (Globals cfg1)) as gs1; destruct gs1; [|done].
    remember (values2GenericValueExt (CurTargetData cfg2) v2_tl olc2 lc2 (Globals cfg2)) as gs2; destruct gs2; [|done].
    inv H1; inv H4.
...
Lemma self_use_check_preserves_getOperandValue:
  forall i v td lc gl gvs rv
    ...
    getOperandValue td v (updateAddAL _ lc i gvs) gl = rv.
Proof.
  intros.
...
Lemma self_use_check_preserves_select_getOperandValue_1:
  forall i cv t v1 v2 td lc gl gvs
    ...
    getOperandValue td cv (updateAddAL _ lc i gvs) gl.
...
Lemma self_use_check_preserves_select_getOperandValue_2:
  forall i cv t v1 v2 td lc gl gvs
    ...
    getOperandValue td v1 (updateAddAL _ lc i gvs) gl.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_select_getOperandValue_3:
  forall i cv t v1 v2 td lc gl gvs
    ...
    getOperandValue td v2 (updateAddAL _ lc i gvs) gl.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_load_getOperandValue:
  forall i t v a td lc gl gvs
    ...
    getOperandValue td v (updateAddAL _ lc i gvs) gl.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_insertValue_getOperandValue_1:
  forall i t1 v1 t2 v2 lconst td lc gl gvs rv
    ...
    getOperandValue td v1 (updateAddAL _ lc i gvs) gl = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_insertValue_getOperandValue_2:
  forall i t1 v1 t2 v2 lconst td lc gl gvs rv
    ...
    getOperandValue td v2 (updateAddAL _ lc i gvs) gl = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_gep_getOperandValue_2:
  forall i inb t1 v lsv t2 td lc gl gvs rv
    ...
    values2GVs td lsv (updateAddAL _ lc i gvs) gl = rv.
Proof.
  self_use_check_tac.
  ...
  assert (Haeq: getOperandValue td v0 lc gl =
    getOperandValue td v0 (updateAddAL _ lc i0 gvs) gl)
...
Lemma self_use_check_preserves_BOP:
  forall i b sz value1 value2 lc gl gvs td rv
    ...
    BOP td (updateAddAL _ lc i gvs) gl b sz value1 value2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_FBOP:
  forall i b sz value1 value2 lc gl gvs td rv
    ...
    FBOP td (updateAddAL _ lc i gvs) gl b sz value1 value2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_TRUNC:
  forall i tcop t v ta td lc gl gvs rv
    ...
    TRUNC td (updateAddAL _ lc i gvs) gl tcop t v ta = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_EXT:
  forall i op t1 v t2 td lc gl gvs rv
    ...
    EXT td (updateAddAL _ lc i gvs) gl op t1 v t2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_CAST:
  forall i op t1 v t2 td lc gl gvs rv
    ...
    CAST td (updateAddAL _ lc i gvs) gl op t1 v t2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_ICMP:
  forall i c t v1 v2 td lc gl gvs rv
    ...
    ICMP td (updateAddAL _ lc i gvs) gl c t v1 v2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma self_use_check_preserves_FCMP:
  forall i c t v1 v2 td lc gl gvs rv
    ...
    FCMP td (updateAddAL _ lc i gvs) gl c t v1 v2 = rv.
Proof. self_use_check_tac. Qed.
...
Lemma not_in_maydiff_implies_gv_inject:
  forall ii md lc1 lc2 i0 igvs1 igvs2 alpha olc1 olc2
    ...
    (Hmdsem: maydiff_sem (updateAddAL _ lc1 i0 igvs1)
      (updateAddAL _ lc2 i0 igvs2) alpha olc1 olc2
      (IdExtSetImpl.add (vars_aux.add_ntag i0)
        (IdExtSetImpl.add (vars_aux.add_ntag i0) md)))
    (Hneqi1 : i0 <> ii),
    (lookupAL _ lc1 ii = None /\ lookupAL _ lc2 ii = None) \/
    (exists gvs1, exists gvs2,
      lookupAL _ lc1 ii = Some gvs1 /\ lookupAL _ lc2 ii = Some gvs2 /\
...
Proof.
  ...
  rewrite <- lookupAL_updateAddAL_neq in Hveq; [|by intro Hf; symmetry in Hf].
  destruct (lookupAL _ lc1 ii), (lookupAL _ lc2 ii); try done.
...
Lemma getOperandValue_eq_check_value_implies_injection:
  forall md inv1 inv2 iso1 iso2 v1 v2 lc1 lc2 alpha gmax li1 li2 olc1 olc2 gvs1 gvs2
    ...
    (Hmdsem: maydiff_sem (updateAddAL _ lc1 i igvs1) (updateAddAL _ lc2 i igvs2)
      alpha olc1 olc2 
      (IdExtSetImpl.add (vars_aux.add_ntag i)
        (IdExtSetImpl.add (vars_aux.add_ntag i) md)))
    (Hinvsem: invariant_sem cfg1 cfg2
      (updateAddAL _ lc1 i igvs1) (updateAddAL _ lc2 i igvs2) mem1 mem2
...
Lemma malloc_preserves_isolated_sem_2:
  forall td iso olc lc mem mem' li tsz gn aln mb i
    ...
    isolated_sem td olc (updateAddAL _ lc i (blk2GV td mb)) (mb::li) iso.
Proof.
  intros.
  ...
  forall md inv1 inv2 t cv1 lv1 rv1 cv2 lv2 rv2 lc1 lc2 alpha gmax li1 li2 olc1 olc2
    ...
    (Hmdsem: maydiff_sem
      (if isGVZero (CurTargetData cfg1) cgvs1
        then updateAddAL _ lc1 i rgvs1
        else updateAddAL _ lc1 i lgvs1)
      (if isGVZero (CurTargetData cfg2) cgvs2
        then updateAddAL _ lc2 i rgvs2
        else updateAddAL _ lc2 i lgvs2)
      alpha olc1 olc2 
      (IdExtSetImpl.add (vars_aux.add_ntag i)
        (IdExtSetImpl.add (vars_aux.add_ntag i) md)))
    (Hinvsem: invariant_sem cfg1 cfg2
    ...
    match
      ...
      (if isGVZero (CurTargetData cfg1) cgvs1
        then updateAddAL _ lc1 i rgvs1
        else updateAddAL _ lc1 i lgvs1) i
      with
      | ret lrgvs1 =>
        match
          lookupAL _
          (if isGVZero (CurTargetData cfg2) cgvs2
            then updateAddAL _ lc2 i rgvs2
            else updateAddAL _ lc2 i lgvs2) i
          with

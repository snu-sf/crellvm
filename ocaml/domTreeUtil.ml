...
let rec bfs_tree (dtree: 'a coq_DTree) : 'a list = 
  match dtree with
  | DT_node (a, dtrees) -> a :: (bfs_trees dtrees)
and bfs_trees (dtrees: 'a coq_DTrees) : 'a list = 
  match dtrees with
  | DT_nil -> []
  | DT_cons (dtree, dtrees) ->
    let l1 = bfs_tree dtree in
    let l2 = bfs_trees dtrees in
    List.append l1 l2

...
Fixpoint valid_cmds
  ...
  | (infrules, inv)::hint, cmd_src::src, cmd_tgt::tgt =>
    ...
    | Some inv1 =>
      ...
      else failwith_None "valid_cmds: Invariant.implies returned false"
    end
...
Definition valid_terminator
  ...
  | insn_return _ ty_src val_src, insn_return _ ty_tgt val_tgt =>
    ...
    then failwith_false "valid_terminator: return type not matched"
    else
    if negb (Invariant.inject_value
               inv0
               (ValueT.lift Tag.physical val_src)
               (ValueT.lift Tag.physical val_tgt))
    then failwith_false "valid_terminator: inject_value of returned values failed"
    else true
  ...
  | insn_br _ val_src l1_src l2_src, insn_br _ val_tgt l1_tgt l2_tgt =>
    if negb (Invariant.inject_value
               ...
               (ValueT.lift Tag.physical val_src)
               (ValueT.lift Tag.physical val_tgt))
    then failwith_false "valid_terminator: inject_value of branch conditions failed"
    else
    if negb (l_dec l1_src l1_tgt)
    then failwith_false "valid_terminator: labels of true branches not matched"
    else
    if negb (l_dec l2_src l2_tgt)
    then failwith_false "valid_terminator: labels of false branches not matched"
    else
    if negb (valid_phinodes hint_fdef inv0 blocks_src blocks_tgt bid l1_src)
    then failwith_false "valid_terminator: valid_phinodes of true branches failed"
    else
    if negb (valid_phinodes hint_fdef inv0 blocks_src blocks_tgt bid l2_src)
    then failwith_false "valid_terminator: valid_phinodes of false branches failed"
    else true
    
  | insn_br_uncond _ l_src, insn_br_uncond _ l_tgt =>
    if negb (l_dec l_src l_tgt)
    then failwith_false "valid_terminator: labels of unconditional branches not matched"
    else
    if negb (valid_phinodes hint_fdef inv0 blocks_src blocks_tgt bid l_src)
    then failwith_false "valid_terminator: valid_phinodes of unconditional branches failed"
    else true
    
  | insn_unreachable _, insn_unreachable _ => true
  | _, _ => failwith_false "valid_terminator: types of terminators not matched"
  end.
...
Definition valid_stmts
  ...
  | None => failwith_false "valid_stmts: valid_cmds failed"
  | Some inv =>
    if negb (valid_terminator hint_fdef inv blocks_src blocks_tgt bid terminator_src terminator_tgt)
    then failwith_false "valid_stmts: valid_terminator failed"
    else true              
  end.
...
Definition valid_entry_stmts (src tgt:stmts) (hint:ValidationHint.stmts): bool :=
  ...
  then failwith_false "valid_entry_stmts: phinode of source not empty"
  else
  if negb (is_empty phinodes_tgt)
  then failwith_false "valid_entry_stmts: phinode of target not empty"
  else
  if negb (Invariant.is_empty_unary hint.(ValidationHint.invariant_after_phinodes).(Invariant.src))
  then failwith_false "valid_entry_stmts: invariant_after_phinodes of source not empty"
  else
  if negb (Invariant.is_empty_unary hint.(ValidationHint.invariant_after_phinodes).(Invariant.tgt))
  then failwith_false "valid_entry_stmts: invariant_after_phinodes of target not empty"
  else true
  .
...
Definition valid_fdef
  ...
  then failwith_false "valid_fdef: function headers not matched"
  else
  match blocks_src, blocks_tgt with
  | (bid_src, block_src)::_, (bid_tgt, block_tgt)::_ =>
    if negb (id_dec bid_src bid_tgt)
    then failwith_false "valid_fdef: entry block ids not matched"
    else
    match lookupAL _ hint bid_src with
    | Some hint_stmts =>
      if negb (valid_entry_stmts block_src block_tgt hint_stmts)
      then failwith_false "valid_fdef: valid_entry_stmts failed"
      else true
             
    | None => failwith_false "valid_fdef: entry block hint not exist"
    end
      
  | _, _ => failwith_false "valid_fdef: empty source or target block"
  end &&
  forallb2AL
    (fun bid stmts_src stmts_tgt =>
       ...
       | Some hint_stmts =>
         ...
         then failwith_false "valid_fdef: valid_stmts failed"
         else true

       | None => failwith_false "valid_fdef: block hint not exist"
       end)
...
Definition valid_product (hint:ValidationHint.products) (src tgt:product): bool :=
  ...
  | product_gvar gvar_src, product_gvar gvar_tgt =>
    ...
    then failwith_false "valid_product: global variables not matched"
    else true
  | product_fdec fdec_src, product_fdec fdec_tgt =>
    if negb (fdec_dec fdec_src fdec_tgt)
    then failwith_false "valid_product: function declarations not matched"
    else true       
  | product_fdef fdef_src, product_fdef fdef_tgt =>
    ...
    then failwith_false "valid_product: function ids not matched"
    else 
    match lookupAL _ hint fid_src with
    | None => failwith_false "valid_product: hint of current function not exist"
    | Some hint_fdef =>
      if negb (valid_fdef fdef_src fdef_tgt hint_fdef)
      then failwith_false "valid_product: valid_fdef failed"
      else true  
    end
  | _, _ =>
    failwith_false "valid_product: source and target product types not matched"
  end.
...
Definition valid_module (hint:ValidationHint.module) (src tgt:module): bool :=
  ...
  then failwith_false "valid_module: layouts not matched"
  else
  if negb (namedts_dec namedts_src namedts_tgt)
  then failwith_false "valid_module: named types not matched"
  else
  if negb (valid_products hint products_src products_tgt)
  then failwith_false "valid_module: valid_products failed"
  else true.
